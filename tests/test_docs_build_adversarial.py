...
class DeepDivePageMapRealTreeTests(unittest.TestCase):
    ...
    def test_every_real_docs_md_source_is_mapped_with_lowercased_slug(self):
        ...
        self.assertEqual(len(md_sources), 26)  # 2026-09-13: step 23 added docs/CURSOR-HARNESS.md (25 -> 26 sources)
    ...
    def test_page_map_has_exactly_28_entries(self):
        """26 docs/*.md sources + 2 .html companions = 28 keys; the .html
        files themselves never get an extra mirror page (27 pages: 26
...
class DeepDiveGlobDerivationExtraFileTests(unittest.TestCase):
    ...
    expected_pages() must grow by exactly one mirror page (27 -> 28 deep-dive
    pages; 70 -> 71 total), never touching the real tracked docs/ dir.
    2026-09-13: baseline 69 -> 70 when step 23 added docs/CURSOR-HARNESS.md."""

    def test_extra_doc_file_yields_a_28th_mirror_page(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = _copy_real_dirs(tmp, ("skills", "copilot", "codex", "docs"))
            baseline = db.expected_pages(root)
            self.assertEqual(len(baseline), 70)
