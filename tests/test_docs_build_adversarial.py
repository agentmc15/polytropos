...
class DeepDivePageMapRealTreeTests(unittest.TestCase):
    ...
    def test_every_real_docs_md_source_is_mapped_with_lowercased_slug(self):
        ...
        self.assertEqual(len(md_sources), 24)
    ...
    def test_page_map_has_exactly_26_entries(self):
        """24 docs/*.md sources + 2 .html companions = 26 keys; the .html
        files themselves never get an extra mirror page (25 pages: 24
        mirrors + 1 index, per the brief's 'Currently 24 sources -> 25
        pages')."""
        page_map = db.deep_dive_page_map(REPO_ROOT)
        self.assertEqual(len(page_map), 26)
...
class DeepDiveGlobDerivationExtraFileTests(unittest.TestCase):
    ...
    expected_pages() must grow by exactly one mirror page (25 -> 26 deep-dive
    pages; 68 -> 69 total), never touching the real tracked docs/ dir."""

    def test_extra_doc_file_yields_a_26th_mirror_page(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = _copy_real_dirs(tmp, ("skills", "copilot", "codex", "docs"))
            baseline = db.expected_pages(root)
            self.assertEqual(len(baseline), 68)
