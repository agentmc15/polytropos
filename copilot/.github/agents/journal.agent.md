...
dispatches the Claude CLI to write the documents itself — a cross-harness spend this agent must
...
folds them into the digest. For tools the collector cannot see (Copilot Studio, Teams, Outlook),
generate offline, ready-to-paste prompts:

```bash
python3 {{POLYTROPOS_ROOT}}/bin/journal_askpack.py --date <date> --print
```

Run each printed prompt inside that tool's own AI, then paste the bullet results back into
`journal/inbox.md`; re-collect afterward so the enriched inbox flows into the digest. No
network, OAuth, Graph, or MCP call is ever added to fetch this content — it is offline text
generation only, carried over by hand.

## Next-day runbook

Once a digest exists, build tomorrow's dated, checkable plan:

```bash
python3 {{POLYTROPOS_ROOT}}/bin/journal_plan.py build
```

Enrich the What/How bodies in this session (same paid-session precedent as the summaries):
...
byte-identical), and save the revision back over the same runbook file. Use
`journal_plan.py check` / `done <id>` / `defer <id> --to <date>` to track cards. Advisory only —
the runbook prepares and tracks; it never schedules or executes anything.
