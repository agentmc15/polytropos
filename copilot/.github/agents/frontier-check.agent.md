...
number. If the user's plan is known, add:

```bash
python3 {{POLYTROPOS_ROOT}}/bin/copilot_pricing.py runway <PLAN> <PROFILE> <MODEL_ID>
```

to show what share of their monthly allowance one run of the frontier-tier model burns. AI
Credits are money — the frontier tier is the most expensive lane on the roster by design, so
the recommendation must say what makes this particular task worth that spend.
...
1. **Full spec up front.** One well-specified turn — goal, constraints, what "done" looks
   like — beats drip-fed instructions; long-horizon coherence comes from planning against a
   clear goal.
...
| per-agent pin | `model:` frontmatter in a `.github/agents/*.agent.md` file |

## Standing recommendation

For multi-task frontier-class work, prefer the `architect` agent: the frontier-tier model
plans once into `tasks/kits/<slug>/`, and cheaper tiers execute the resulting tasks — the
frontier spend concentrates in the short planning phase. For a single verify-gated task, use
the `escalate` agent instead.
