<!-- GENERATED by bin/docs_build.py from docs/GRAPH-ENGINEERING.md — do not edit; edit the source and run: python3 bin/docs_build.py build -->

# Graph-Engineering Properties in polytropos

!!! note
    Mirrored from `docs/GRAPH-ENGINEERING.md` — edit the source, then run `python3 bin/docs_build.py build`.
...
None of that is immutability, and this document should not be read as claiming it. With `Bash` in the pin, a role can still reach anything a shell can reach. The only mechanism here that constrains that is the OS boundary in `bin/exec_policy.py`, which currently covers **verification** (macOS only; see [SECURITY.md](https://github.com/agentmc15/polytropos/blob/main/SECURITY.md)). Review dispatch itself is not yet confined.
