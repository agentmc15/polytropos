...
`primitives/harness-capabilities.json`); as of this writing the latter exists for Claude Code and
for Cursor's CLI (one live run and one read-only review each, on throwaway kits, 2026-09-16), and
every unrun live check -- Codex, Copilot, the workflow evaluation -- is listed there as prepared,
not done.
