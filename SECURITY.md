...
`primitives/harness-capabilities.json`); as of this writing the latter exists for Claude Code (one
live run and one review on a throwaway kit, 2026-09-16) and for Cursor's identity probe alone
(the CLI was installed the same day but not logged in, so its dispatch stays unknown), and every
unrun live check is listed there as prepared, not done.
