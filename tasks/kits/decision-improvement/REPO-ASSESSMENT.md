...
**Plan revision, D01, 2026-09-16 — recorded, not repaired.** Both are in `docs/DECISION-IMPROVEMENT-RECONCILIATION.md` as `BL1` and `BL2`, with the exact mechanism of each. `BL1` reproduced at D01's hand-back and was then the full suite's single failure: `test_lessons_promote.NoScaffoldingWritesTests.test_real_run_touches_only_gitignored_path` built its expected path from the repository root while `bin/lessons_promote.py:93` resolves it through `bin/runtime_data.py`, which returns the in-tree location only when it exists and has content — so the product is obeying the outside-the-tree store invariant and the test is asserting a checkout shape. The earlier empty-fixture accommodation was not repeated. `BL2` did not reproduce here, which proves nothing about the race. Neither was touched by D01, which changed no test. **Correction, phase 1 review, 2026-09-16: `BL1` has since been repaired** by the executor outside any task — the test now resolves its expected path through `runtime_data`'s own resolver with `POLYTROPOS_DATA_HOME` pinned to a temp dir, and the suite is green. The repair made the guard strictly stronger: the old assertion would have PASSED a product mutated to write inside the tree, which is an invariant violation. Do not read the present tense above as current.
