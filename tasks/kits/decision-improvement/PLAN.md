...
**Plan revision, D01, 2026-09-16 — both are recorded, neither is repaired.** The lessons-promotion limitation reproduces on this HEAD and is the full suite's single failure; its exact mechanism, and why the product rather than the test is correct, is in the reconciliation under `BL1`. The scheduler race did not reproduce here, which proves nothing about it; it is recorded as `BL2`. Repairs belong to those surfaces' own owners and to their own scope, never folded into a decision-improvement task.
