...
**Plan revision, D01, 2026-09-16 — those commits have landed.** The reconciliation ran against HEAD `e6cf4bd6c86daac53ac99d4d484c4bb802b84bb5`, a merge whose second parent is `8d1b7be`; `b48a284`, `1755ead` and `fd80734` are reachable from it, together with `ab2a42a`, `e23f2ae` and `8d1b7be`. All four installed clients now carry dated `verified: supported` rows from 2026-09-16 with their client versions recorded, where the baseline had only Claude Code and an unknown Cursor. No brief was found to contain a claim those commits falsify, so no task brief was rewritten; the paragraph above stands as preparation history and the present-tense picture, the per-step classification of original steps 16–26, and the remaining deltas are in `docs/DECISION-IMPROVEMENT-RECONCILIATION.md` and its JSON twin. The distinction it insists on is unchanged: source capability support is still not installed-client verification, and `unknown` in `primitives/harness-capabilities.json` still means no.
...
**Plan revision, D01, 2026-09-16, superseded in part — see the next sentence.** The lessons-promotion limitation reproduced at D01's hand-back and was then the full suite's single failure; its exact mechanism, and why the product rather than the test is correct, is in the reconciliation under `BL1`. The scheduler race did not reproduce here, which proves nothing about it; it is recorded as `BL2`. **Correction, phase 1 review, 2026-09-16: `BL1` HAS since been repaired** by the executor, outside any task and recorded as a `defect:` line in the kit's NOTES.md. The test now resolves its expected path through `bin/runtime_data.py`'s own resolver with `POLYTROPOS_DATA_HOME` pinned to a temp dir, so it is correct in a fresh worktree and in a populated checkout alike and can never reach a real store. The full suite is green; do NOT read the sentence above as current. `BL2` remains unrepaired and unreproduced. Repairs otherwise belong to those surfaces' own owners and to their own scope, never folded into a decision-improvement task.
