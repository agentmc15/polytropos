...
In Cursor this skill is project-scoped: it lives at `.cursor/skills/assess-improvement/` and the same files are read by the editor agent, the CLI, and cloud subagents. It drives nothing. It dispatches no model, runs no command, and names no CLI to invoke; the linked template was installed beside it.
