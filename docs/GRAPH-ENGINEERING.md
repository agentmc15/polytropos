...
|---|---|---|---|---|
| Node isolation / tool scoping | models pinned; declared pin now reaches dispatch | agents exist; review no longer gets a blanket grant | no tool scoping; read-only sandbox on review/acceptance | scoped where the harness allows |
...
- **Codex** — no tool-scoping surface exists. The Codex roles are prompt preambles (`codex/prompts/{implementer,verifier,reviewer,…}.md`) whose frontmatter carries a description and nothing else, and PLAN.md's out-of-scope list fences Codex agent files out for exactly that reason. Codex does, however, have something the other two lacked: `bin/codex_execute.py` pins `--sandbox read-only` on review and acceptance and refuses any attempt to override it through extra arguments. That is an **execution boundary**, not tool scoping, and the distinction is the point of the paragraph below — Codex constrains what a role can DO to the filesystem without constraining which tools it may name.
...
**Three different properties, and they are not substitutes.** A *tool pin* says which tools a role may name. A *permission grant* says whether those calls need approval. An *execution boundary* says what the process can reach regardless of either. Until recently the first two were declared and then discarded: `claude_execute` stripped the frontmatter carrying the pin and appended a blanket permission grant to every dispatch, and Copilot review received `--allow-all-tools`, so a role documented as read-only was dispatched with approval for everything including command execution. Both now honour the declaration — the Claude pin is translated to `--allowedTools` and neither review path receives a blanket grant, with `--review-permissions bypass` as the named, reported opt-out.

None of that is immutability, and this document should not be read as claiming it. With `Bash` in the pin, a role can still reach anything a shell can reach. The only mechanism here that constrains that is the OS boundary in `bin/exec_policy.py`, which currently covers **verification** (macOS only; see [SECURITY.md](../SECURITY.md)). Review dispatch itself is not yet confined.
...
- **Ids and lineage in the outcome grammar.** The shared `outcome:` ledger line gained three optional fields — `run=`, `parent=`, and `failure=` (one of `execution`, `coherence`, `verification`) — plus a fifth `result=` value, `budget-stop`. Every one is optional and absent-means-today's-behavior: a kit executed before they existed parses and scores exactly as it always did, which the byte-stability goldens in `tests/test_routing_scorecard.py` pin on both the markdown and the JSON surfaces. A kit's `NOTES.md` is the canonical ledger. Since roadmap step 17 it is one of three sources `bin/attempt_history.py` joins into a single record per attempt — beside the attempt ledger (every dispatch, including Claude and Copilot reviews, which used to leave nothing but stdout) and Codex's typed `role-use.jsonl` — with every unobserved field kept `None` and counted, history never collapsed to last-wins (the latest task state is a separate projection), a failed consult keeping its parent, and cost carried per basis (billed / credits / estimated / proxy / model-reported) and never summed across bases. Concrete model ids on any line resolve through `bin/model_registry.py`, the one cross-harness reader of ids and tiers, so a pass on `claude-opus-5` counts under `opus` instead of falling off the alias ladder; an id no pricing file knows still reads as no evidence.
