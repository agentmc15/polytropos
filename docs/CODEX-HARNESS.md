...
**Two named routing policies.** Everything above is the `reserved` policy, and it stays the default: workers implement, Astra is held for acceptance and evidence-gated recovery, a frontier plan migrates down, one command-line effort serves every rung. Since roadmap step 19 it is selectable by name beside an opt-in `adaptive` policy that may send a hard task straight to an eligible capable model, the frontier included, without a failed cheaper attempt first. Select it per run (`run --policy adaptive --preference quality`) or per kit with a PLAN.md line in the same family as `budget:` (`routing: policy=adaptive preference=cost profile=M`); the flag outranks the line, and neither given means `reserved`. The decision separates four things — workflow shape (`direct`, `reviewed`, `graph`), model, initial effort, and assurance — and is explainable from its own record: `prepare --policy adaptive --model strong --explain` (or `run --dry-run`) prints the floor and the signals that set it, the model with its rank, the effort and why (adaptive may start a difficult task one step above the model's own default when the host reports that step), every alternative with the filter that removed it, the ladder, an `est.` API-equivalent whole-workflow figure per stage when a profile is given (never a bill), and the shape's capabilities that are unverified on this host. Hard filters come before any preference: availability, routability, an explicit pin, an exclusion, an unsupported effort, and the budget all apply first, and `cost`/`quality`/`balanced`/`latency` only choose among what survived (`latency` says it has no data). Under `adaptive` an unavailable Astra disables nothing a worker path does not need — the same request under `reserved` still fails closed, as it always has. Under both policies a dispatch that failed for an `auth`, `config`, `permission`, or `infrastructure` reason stops the ladder and names the class instead of climbing; a bare non-zero exit still climbs. The policy in force is written to the run's NOTES.md block as a `- routing:` line. Which policy is active is an explicit selection, never learned from observations; installed defaults are not changed. The same decision contract routes the other harnesses' rosters by rank rather than tier name (`python3 bin/routing_policy.py decide --harness claude --planned opus --policy adaptive`), though only the Codex driver dispatches under it today.
