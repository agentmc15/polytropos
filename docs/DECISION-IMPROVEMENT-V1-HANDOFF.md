...
| Routing policy (`workflow_eval.POLICY_FILE` under the `prefs` store) | `python3 bin/workflow_eval.py propose`, then `review`, then `apply`; every version kept | `python3 bin/workflow_eval.py rollback --to N`; the replaced version is kept too |
