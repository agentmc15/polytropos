# aesop compile round-trip — a proposal (ARCHIVED, kept as history)

> # ⚠ ARCHIVED — this specification will not be executed
>
> Two independent reasons, either one sufficient:
>
> 1. **Superseded.** The aesop-fold kit changed the direction from "make the Copilot bundle an
>    `aesop compile` target" to "fold aesop's primitive model into polytropos" — see the status
>    note below and `docs/PRIMITIVES.md`.
> 2. **The target repo no longer exists as a live project.** **aesop was archived on 2026-09-05** —
>    its compiler and emitters stayed behind; what it got right came here. This document is written
>    to be the input to "a future `/polytropos:architect` run executed **inside the aesop repo**"
>    (see "Execution: a future architect run inside the aesop repo" near the end). That run cannot
>    happen, so every aesop-side option, reconciliation, and round-trip specified below is
>    unreachable rather than pending.
>
> Nothing below has been edited to match: it is kept verbatim as the record of what was specified
> and why, including the pins to `aesop@5506617` and the one half that did ship upstream (PR #1,
> noted two blocks down). Read it as history. It is not a backlog, and no part of it is a task.
> The file is deliberately kept in place rather than deleted — the docs generators' census and the
> site navigation both reference it.

> **Status (2026-09-04, pre-dating the archival above): superseded by the aesop-fold kit.** The direction changed from "make the Copilot bundle an `aesop compile` target" to "fold aesop's primitive model into polytropos": the manifest is now `copilot/aesop.toml` (TOML dialect of the same v1 schema), validated offline by `python3 bin/primitives.py check`, and `aesop compile` is not planned to run here. The specification below is kept as history; see `docs/PRIMITIVES.md` and `.claude/kits/aesop-fold/PLAN.md` (Evaluation) for the standing decision.
